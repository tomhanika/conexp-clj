...
~fca-clj~ is a standalone console application that supports some of ~conexp-clj~ function and can conveniently be run from the command line.
It can be downloaded here:	[[https://github.com/jana-fischer/fca-clj]]
...
The ~fca-clj~ repository provides a detailed documentation.
