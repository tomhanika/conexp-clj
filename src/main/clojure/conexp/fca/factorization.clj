...
(defn- grecond-test-remainder-match
  [A U]
  (loop [i 0 c 0 u U]
    (if (>= i (count A))
      {:u u :c c}
    ...
    (if (> i (- (count S) 1))
      ...
      (recur 
        ...
        (cond (= (* (count (nth (nth S i) 0)) (count (nth (nth S i) 1)))
                 (:c (grecond-test-full-match (grecond-make-matrix-from-concept (nth (nth S i) 0) (nth (nth S i) 1) n m) u)))
          (:u (grecond-test-full-match (grecond-make-matrix-from-concept (nth (nth S i) 0) (nth (nth S i) 1) n m) u)) :else u)
        (cond (= (* (count (nth (nth S i) 0)) (count (nth (nth S i) 1)))
                 (:c (grecond-test-full-match (grecond-make-matrix-from-concept (nth (nth S i) 0) (nth (nth S i) 1) n m) u)))
  ...
  (loop [i 0 u U f F]
    (if (< k i)
      ...
      (recur 
        (inc i)
        (cond
          (< 0 (:c (grecond-test-remainder-match (grecond-make-matrix-from-concept (nth (nth S i) 0) 
                                                                            (nth (nth S i) 1)
                                                                            n
                                                                            m)
                                              u)))
          (:u (grecond-test-remainder-match (grecond-make-matrix-from-concept (nth (nth S i) 0)
                                                                       (nth (nth S i) 1)
                                                                       n
                                                                       m)
                                         u))
...
(defn grecond
  ...
  (let [S (reverse (sort-by (fn [[a b]] (* (count a) (count b))) (grecond-create-usable (concepts I))))
    ...
    (let [full (grecond-full-match S (into [] (apply concat U)) (count U) (count (get U 0)))]
