...
(defprotocol Factorization 
  (context [this] "Returns the context that is generated after using the panda algo k-times on it")
  (object-factor [this] "Return the Factorization matrices of the context generated by the panda-algo")
  (attribute-factor [this]))

(defrecord factorization [algo]
  Factorization
  (context [this] (get algo 0))
  (object-factor [this] (get algo 1))
  (attribute-factor [this] (get algo 2)))
...
(defn- makeObject
  ...
  (loop [i 0 out []]
    (if (<= k i)
      ...
      (recur (inc i) (conj out (makeObjectHelper concept m k i))))))
  ...
  (let [E (get C :e)]
    (loop [i 0 item (nth E 0) CS C]
      (if (<= (count E) i)
        ...
        (recur (inc i)
               (nth E i)
               (newTransactions (weight CS {:ci (assoc (get CS :ci) item 1) :ct (get CS :ct)} D)
                                pi
                                D))))))

(defn- pandaFactorizationMatrices
...
(defn panda
  ...
  (let [D (make-matrix-from-context (context-to-string input))]
    (loop [i 0 pi (list) cur-D D]
      (if (<= k i)
        (pandaFactorizationMatrices pi (count (get (nth pi 0) :ci)) (count (get (nth pi 0) :ct)) k)
        (recur (inc i)
               (conj pi (extendCore (findCore cur-D) pi cur-D))
               (createNewD (calcOneMatrix (get (extendCore (findCore cur-D) pi cur-D) :ci) 
                                          (get (extendCore (findCore cur-D) pi cur-D) :ct))
                           cur-D))))))
...
(defn- grecondRemainderMatch
  ...
  (loop [i 0 u U f F]
    (if (<= k i)
      [(calcGrecondContext f m n) (makeObject f m k) (makeAttribute f n k)]
      (recur (inc i)
...
(defn- calcNewCoverageHyper
  ...
  (let [X (apply min 
                (map :cost
                     (into [] (flatten (calcCostListHyper C Coverage)))))
...
(defn hyper
  ...
  (let [n (count(nth (make-matrix-from-context (context-to-string Input))0))
    ...
    (loop [Output []
        ...
        (if (>= i k)
        [(calcHyperContext [Output Coverage]) (makeObjectHyper Output m k) (makeAttributeHyper Output n k)]
...
(defn greess
  ...
  (let [gamma (calcDerivation Input attributes attribute-derivation)
    ...
    (loopOverlaps (grecondCreateUsable (concepts Input))
                  (remove nil? (markGreess (make-matrix-from-context (context-to-string Input)) [gamma mu]))
                  k
                  (count U)
                  (count (get U 0)))))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
